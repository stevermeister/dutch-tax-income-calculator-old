...
const calcComponent = {
  ...
  controller: function($scope, $location) {
    ...
    if (
      ...
      this.years.indexOf(+$location.search().year) !== -1
    ) {
    ...
    this.salary = {
      ...
      socialSecurity: !(
        angular.isDefined($location.search().socialSecurity) &&
        $location.search().socialSecurity === '0'
      ),
    ...
    $scope.$watchGroup(
      ...
      () => {
        ...
        let grossYear =
          salary.grossYear +
          salary.grossMonth * 12 +
          salary.grossWeek * constants.workingWeeks;
        grossYear +=
          salary.grossDay * constants.workingDays +
          salary.grossHour * constants.workingWeeks * salary.hours;
        if (!grossYear || grossYear < 0) {
          grossYear = 0;
        }

        salary.grossAllowance = salary.allowance
          ? ~~(grossYear * (0.08 / 1.08))
          : 0; // Vakantiegeld (8%)
        ...
        salary.grossHour = ~~(
          grossYear /
          (constants.workingWeeks * salary.hours)
        );

        salary.taxFreeYear = 0;
        salary.taxableYear = grossYear - salary.grossAllowance;
        if (this.ruling.checked) {
          const rulingIncome = getRulingIncome(this.year, this.ruling.choice);
          if (salary.taxableYear > rulingIncome) {
            salary.taxFreeYear = salary.taxableYear * 0.3;
        ...
        salary.socialTax = salary.socialSecurity
          ? -1 * getSocialTax(this.year, salary.taxableYear, salary.older)
          : 0;
        const socialCredit = getSocialCredit(
          this.year,
          salary.older,
          salary.socialSecurity,
        );
        salary.generalCredit =
          socialCredit *
          getGeneralCredit(this.year, salary.taxableYear, salary.older);
        salary.labourCredit =
          socialCredit *
          getLabourCredit(this.year, salary.taxableYear, salary.older);
        salary.incomeTax = ~~(
          salary.payrollTax +
          salary.socialTax +
          salary.generalCredit +
        ...
        salary.netYear =
          salary.taxableYear + salary.incomeTax + salary.taxFreeYear;
        salary.netAllowance = salary.allowance
          ? ~~(salary.netYear * (0.08 / 1.08))
          : 0;
    ...
    function getSocialCredit(year, older, socialSecurity) {
       ...
       */
      const bracket = constants.socialPercent[year][0];
      let percentage = 1;
    ...
    function getRates(brackets, salary, type) {
      ...
      brackets.some((bracket, index) => {
        ...
        percent = parseInt(tax) !== 0 && tax > -1 && tax < 1; // Check if rate is percentage or fixed
        if (salary <= delta) {
          if (percent) {
